...
object GsonUtil {
    ...
    val instance: Gson?
        get() {
            if (gson == null) {
                // Register custom types
                val locationMapType = object : TypeToken<Map<String?, Location?>?>() {}.type
                gson = GsonBuilder().excludeFieldsWithoutExposeAnnotation().setPrettyPrinting()
                        .serializeNulls().registerTypeAdapter(Location::class.java, LocationSerializer())
                        .registerTypeAdapter(locationMapType, LocationMapDeserializer())
                        .registerTypeAdapter(locationMapType, LocationMapSerializer())
                        .registerTypeAdapter(Location::class.java, LocationDeserializer()).create()
            }
            return gson
        }
}