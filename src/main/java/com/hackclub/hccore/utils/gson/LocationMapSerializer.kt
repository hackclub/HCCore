...
class LocationMapSerializer : JsonSerializer<Map<String?, Location?>> {
    override fun serialize(source: Map<String?, Location?>, type: Type,
                           context: JsonSerializationContext): JsonElement {
        val jsonLocationMap = JsonObject()
        for ((key, value) in source) {
            jsonLocationMap.add(key, context.serialize(value))
        }
