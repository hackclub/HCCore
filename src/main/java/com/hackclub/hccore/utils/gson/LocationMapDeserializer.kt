...
class LocationMapDeserializer : JsonDeserializer<Map<String, Location>> {
    override fun deserialize(source: JsonElement, type: Type,
                             context: JsonDeserializationContext): Map<String, Location> {
        val jsonObject = source.asJsonObject
        val locationMap: MutableMap<String, Location> = LinkedHashMap()
        for ((key, value) in jsonObject.entrySet()) {
            locationMap[key] = context.deserialize(value, Location::class.java)
        }
        return locationMap
    }
}