...
class LocationDeserializer : JsonDeserializer<Location> {
    override fun deserialize(source: JsonElement, type: Type, context: JsonDeserializationContext): Location {
        val jsonObject = source.asJsonObject
        val world = jsonObject["world"].asString
        val x = jsonObject["x"].asDouble
        val y = jsonObject["y"].asDouble
        val z = jsonObject["z"].asDouble
        return Location(Bukkit.getServer().getWorld(world), x, y, z)
    }
}