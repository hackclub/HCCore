...
class DownvoteCommand(plugin: HCCorePlugin) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, cmd: Command, alias: String, args: Array<String>): Boolean {
        ...
        val player = sender
        if (args.size == 0) {
            player.chat(DownvoteCommand.Companion.DOWNVOTE)
        } else {
            player.chat(java.lang.String.join(" ", *args) + " " + DownvoteCommand.Companion.DOWNVOTE)
        }
        return true
    }

    companion object {
        private val DOWNVOTE = ChatColor.BLUE.toString() + ChatColor.BOLD.toString() + "↓"
    }
