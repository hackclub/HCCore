...
class NickCommand(plugin: HCCorePlugin) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, cmd: Command, alias: String, args: Array<String>): Boolean {
        ...
        val player = sender
        if (args.size == 0) {
            plugin.getDataManager().getData(player).setNickname(null)
            sender.sendMessage(ChatColor.GREEN.toString() + "Your nickname has been reset!")
            return true
        }
        val newNickname = java.lang.String.join(" ", *args)
        if (newNickname.equals("Saharsh", ignoreCase = true)) {
            plugin.getDataManager().getData(player).setNickname("Saharchery")
            player.kickPlayer("Kicked for being Saharsh.")
            return true
        }
        if (newNickname.length > PlayerData.Companion.MAX_NICKNAME_LENGTH) {
            sender.sendMessage(ChatColor.RED.toString() + "Your nickname can’t be longer than "
                    + PlayerData.Companion.MAX_NICKNAME_LENGTH + " characters")
            return true
        }
        plugin.getDataManager().getData(player).setNickname(newNickname)
        sender.sendMessage(ChatColor.GREEN.toString() + "Your nickname was set to "
                + plugin.getDataManager().getData(player).getNameColor() + newNickname)
        return true
    }
