...
public class SpawnCommand implements CommandExecutor {
    ...
    public boolean onCommand(
        ...
        if (player.getWorld().getEnvironment() != Environment.NORMAL) {
            sender.sendMessage(
                ChatColor.RED + "You can only use this command in the Overworld"
            );
            return true;
        }

        // Player needs to be within the allowed radius from spawn
        int distanceFromSpawn = (int) player
            .getLocation()
            .distance(player.getWorld().getSpawnLocation());
        int allowedRadius =
            this.plugin.getConfig()
                .getInt("settings.spawn-command.allowed-radius");
        if (distanceFromSpawn > allowedRadius) {
            sender.sendMessage(
                ...
                (distanceFromSpawn - allowedRadius) +
                " blocks too far."
            );
            return true;
        }
        ...
        if (!player.isOnGround()) {
            sender.sendMessage(
                ChatColor.RED +
                "You need to be standing on the ground to use this command"
            );
            return true;
        }

        // Player needs to have sky access (no blocks above them at all)
        Block highestBlock = player
            .getWorld()
        ...
        if (player.getLocation().getY() < highestBlock.getY()) {
            sender.sendMessage(
                ChatColor.RED +
                "You need be directly under the sky to use this command"
            );
            return true;
        }
        ...
        long secondsSinceLastDamaged =
            (currentTime - data.getLastDamagedAt()) / 1000;
        int damageCooldown =
            this.plugin.getConfig()
                .getInt("settings.spawn-command.damage-cooldown");
        if (secondsSinceLastDamaged < damageCooldown) {
            sender.sendMessage(
                ChatColor.RED +
                "You were hurt recently—you’ll be able to use this command in " +
                (damageCooldown - secondsSinceLastDamaged) +
                " seconds"
            );
            return true;
        }
        ...
        sender.sendMessage(
            ChatColor.GREEN + "You’ve been teleported to the world spawn"
        );
        return true;
    }
}
