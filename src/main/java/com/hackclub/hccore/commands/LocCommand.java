...
public class LocCommand implements TabExecutor {
    ...
    public boolean onCommand(
        ...
        String locationName = String.join(
            "_",
            ...
            case "del":
                {
                    if (args.length < 2) {
                        sender.sendMessage(
                            ChatColor.RED + "Please specify the location name"
                        );
                        break;
                    }
                    ...
                    if (!data.getSavedLocations().containsKey(locationName)) {
                        sender.sendMessage(
                            ChatColor.RED +
                            "No location with that name was found"
                        );
                        break;
                    }
            ...
            case "get":
                {
                    ...
                    Location savedLocation = data
                        .getSavedLocations()
                        .get(locationName);
                    sender.sendMessage(
                        locationName +
                        ": " +
                        savedLocation.getWorld().getName() +
                        " @ " +
                        savedLocation.getBlockX() +
                        ", " +
                        savedLocation.getBlockY() +
                        ", " +
            ...
            case "share":
                {
                    ...
                    PlayerData recipData =
                        this.plugin.getDataManager().getData(recipient);
                    ...
                    String locationString =
                        "(" +
                        sendLocation.getWorld().getName() +
                        " @ " +
                        sendLocation.getBlockX() +
                        ", " +
                        sendLocation.getBlockY() +
                        ", " +
            ...
            case 2:
                {
                    ...
                    for (Map.Entry<String, Location> entry : data
                        .getSavedLocations()
                        .entrySet()) {
                        if (
