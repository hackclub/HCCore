...
class UpvoteCommand(plugin: HCCorePlugin) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, cmd: Command, alias: String, args: Array<String>): Boolean {
        ...
        val player = sender
        if (args.size == 0) {
            player.chat(UpvoteCommand.Companion.UPVOTE)
        } else {
            player.chat(java.lang.String.join(" ", *args) + " " + UpvoteCommand.Companion.UPVOTE)
        }
        return true
    }

    companion object {
        private val UPVOTE = ChatColor.RED.toString() + ChatColor.BOLD.toString() + "↑"
    }
