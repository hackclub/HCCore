...
class TableflipCommand(plugin: HCCorePlugin) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, cmd: Command, alias: String, args: Array<String>): Boolean {
        ...
        val player = sender
        if (args.size == 0) {
            player.chat(TableflipCommand.Companion.TABLEFLIP)
        } else {
            player.chat(java.lang.String.join(" ", *args) + " " + TableflipCommand.Companion.TABLEFLIP)
        }
        return true
    }
