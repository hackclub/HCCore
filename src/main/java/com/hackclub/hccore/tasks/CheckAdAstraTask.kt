...
class CheckAdAstraTask(private val plugin: HCCorePlugin) : BukkitRunnable() {
    override fun run() {
        for (player in plugin.server.onlinePlayers) {
            ...
            if (player.location.y < MIN_Y) {
                continue
            }
            val key = NamespacedKey(plugin, "ad_astra")
