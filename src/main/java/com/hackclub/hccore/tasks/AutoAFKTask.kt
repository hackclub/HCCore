...
class AutoAFKTask(private val plugin: HCCorePlugin) : BukkitRunnable() {
    override fun run() {
        for (player in plugin.server.onlinePlayers) {
            ...
            val secondsSinceLastActive = (System.currentTimeMillis() - data.lastActiveAt) / 1000
            if (secondsSinceLastActive > plugin.config.getInt("settings.auto-afk-time")) {
                data.isAfk = true
